...
export class OptimizationService {
    public async optimize(
    ...
    ): Promise<OptimizationResult<Dayjs>> {
        if (!loaded) {
            await loadModule();
            loaded = true;
        }
        ...
        const start_of_current_day = start_of_current_hour.subtract(
            time.hour(),
            "hour",
        );

        const current_part_of_hour = time.diff(
            start_of_current_hour,
            "hour",
            true,
        );
        const current_split_hour = Math.floor(
            current_part_of_hour * split_hours_into,
        );
        const current_split_hour_start = start_of_current_hour.add(
            current_split_hour * (60 / split_hours_into),
            "minute",
        );
        ...
        const periods = [...Array(number_of_periods).keys()]
            .map((_, i) => {
                const start =
                    i === 0
                        ? time
                        : current_split_hour_start.add(
                              i * (60 / split_hours_into),
                              "minute",
                          );
                const end = current_split_hour_start.add(
                    (i + 1) * (60 / split_hours_into),
                    "minute",
                );
            ...
            .map((period) => ({
                ...
                rad_flow_temp: Math.max(
                    indoor.target_temp +
                        (indoor.target_temp -
                            (indoor.current_temp !== undefined
                                ? indoor.current_temp
                                : indoor.target_temp)) +
                        (indoor.target_temp -
                            period.outdoor_temp -
                            indoor.passive_heating_degrees) *
                            radiator.c_flow,
                    indoor.target_temp,
                ),
            }))
            .map((period) => ({
                ...
                acc_min_temp: Math.max(
                    period.rad_flow_temp -
                        (acc_spec.radiator_flow_temp_above_acc_avg_temp || 0),
                    hot_water.min_temp,
                    acc_spec.min_temp,
            ...
            .map((period) => {
                const radiator_temp_change =
                    ((period.rad_flow_temp - indoor.target_temp) *
                        radiator.power_per_temp_delta *
                        period.duration) /
                    acc_heat_capacity;
                const hot_water_temp_loss =
                    (hot_water.average_power / acc_heat_capacity) *
                    period.duration;
                const shower_temp_loss =
                    (period.duration * showerX.get_shower(period).power) /
                    acc_heat_capacity;
                return {
                    ...period,
                    consumption: {
        ...
        const acc_temp = periods.map((period, i) =>
            model.addVar({
                ...
                ub:
                    i === 0
                        ? acc_spec.current_temp
                        : Math.min(acc_spec.max_temp, pump_spec.max_temp),
                type: "continuous",
            }),
        );

        const acc_surrounding_diff = periods.map((period) =>
            model.addVar({
                name: `acc_surrounding_diff_${period.start.format(
                    "YYYY-MM-DD_HH:mm",
                )}`,
                type: "continuous",
                obj: 0,
        ...
        periods.forEach((period, period_i) => {
            model.addConstr({
                name: `calc_acc_surrounding_diff_${period.start.format(
                    "YYYY-MM-DD_HH:mm",
                )}`,
                coeffs: [
                    [acc_temp[period_i], 1],
        ...
        const period_pump = periods.map((period) =>
            pump_spec.power.map((powerSpec) =>
                model.addVar({
                    name: `pump_on_${period.start.format(
                        "YYYY-MM-DD_HH:mm",
                    )}_below_${powerSpec.below_degrees || "max"}`,
                    type: "continuous",
                    lb: 0,
                    ub: 1,
                    obj:
                        (period.price *
                            powerSpec.consumed_power_watt *
                            period.duration) /
                        1000,
                }),
            ),
        ...
        periods.slice(0, periods.length - 1).forEach((period, period_i) =>
            model.addConstr({
                name: `acc_temp_change_${period.start.format(
                    "YYYY-MM-DD_HH:mm",
                )}`,
                coeffs: [
                    [acc_temp[period_i], 1],
                    ...
                    [
                        acc_surrounding_diff[period_i],
                        -acc_spec.temp_loss_per_hour_degrees * period.duration,
                    ],
                    ...pumpEffect.heating_per_hour_intervals.map(
                        (hph, i) =>
                            [
                                period_pump[period_i][i],
                                hph.deg * period.duration,
                            ] as [Variable, number],
                    ),
                ],
        ...
        period_pump.forEach((temp_intervals, i) =>
            model.addConstr({
            ...
            pumpEffect.heating_per_hour_intervals
                ...
                .map((main_hph, hph_i) => {
                    const below = model.addVar({
                        name: `acc_below_${period.start.format(
                            "YYYY-MM-DD_HH:mm",
                        )}_${main_hph.max_temp || "max"}`,
                        type: "binary",
                    ...
                    const m_1 = 30;
                    model.addConstr({
                        name: `pump_interval_${period.start.format(
                            "YYYY-MM-DD_HH:mm",
                        )}_${hph_i}_below_${main_hph.max_temp}`,
                        coeffs: [
                            [acc_temp[period_i], 1],
                            [
                                acc_surrounding_diff[period_i],
    ...
    private getResult(
        getStatus: (x?: { statusMIP: any; status: any }) => any,
        ...
        const ok =
            getStatus(model) === "optimal" || getStatus(model) === "feasible";

        const plan = periods.map((period, period_i) => {
            const pump_consumption = pump_spec.power
                .map(
                    (powerSpec, power_i) =>
                        getValue(period_pump[period_i][power_i]) *
                        powerSpec.consumed_power_watt,
                )
                .reduce((prev, curr) => prev + curr, 0);

            const production = pump_spec.power
                .map(
                    (powerSpec, power_i) =>
                        getValue(period_pump[period_i][power_i]) *
                        powerSpec.heating_power_watt,
                )
                .reduce((prev, curr) => prev + curr, 0);

            const nextHourStartTemp =
                getValue(acc_temp[period_i + 1]) ||
                getValue(acc_temp[period_i]);
            ...
            const on_fraction = period_pump[period_i].reduce(
            ...
            const pump: PumpPlan = {
                ...
                cost:
                    (pump_consumption * period.duration * period.price) / 1000,
                production,
                target_temp:
                    on_fraction > 0
                        ? on_fraction > 0.66
            ...
            const consumption_radiator =
                (period.consumption.radiator * acc_heat_capacity) /
                period.duration;
            const consumption_hot_water =
                (period.consumption.hot_water * acc_heat_capacity) /
                period.duration;
            const consumption_shower =
                (period.consumption.shower * acc_heat_capacity) /
                period.duration;
            const consumption_loss =
                acc_spec.temp_loss_per_hour_degrees *
                getValue(acc_surrounding_diff[period_i]) *
                acc_heat_capacity;
...
class PriceExtractor {
    constructor(
        private readonly priceSpec: PriceSpec,
        private readonly startAt: Dayjs,
    ) {
        this.prices = [
            ...priceSpec.today,
            ...(priceSpec.tomorrow || []),
...
class OutdoorTempExtractor {
    ...
    get_temp_at(time: Dayjs): number {
        return this.forecasts.reduce(
            (prevTemp, f) => {
                return f.time.isBefore(time) || f.time.isSame(time)
                    ? f.temp
                    : prevTemp;
            },
...
class PumpEffectExtractor {
    constructor(
        private readonly pumpSpec: PumpSpec,
        private readonly acc_volume: number,
    ) {}

    public readonly heating_per_hour_intervals = this.pumpSpec.power.map(
        (powerSpec, power_i) => ({
    ...
    get_heating_per_hour(i: number) {
        return (
            this.pumpSpec.power[i].heating_power_watt / (this.acc_volume * 1.16)
        );
    }
}
...
class ShowerExtractor {
    ...
    get_shower(period: { start: Dayjs; end: Dayjs; duration: number }): {
        temp: number;
        power: number;
    } {
        if (!this.showers) {
        ...
        const shower = this.showers.find(
            (shower) =>
                !(
                    shower.start.unix() >= period.end.unix() ||
                    shower.end.unix() <= period.start.unix()
                ),
        );
        if (!shower) {
            return { temp: 0, power: 0 };
